...
2. Run the matching command(s) from the terminal and capture stdout:
   - Instant: `mvn compile exec:java`
    ...
    - Deployed container app: do not POST demo API routes; the public web app intentionally exposes no `/api/*` JSON endpoints, no demo action forms, no buttons, and no browser JavaScript model-call flow. Use the CLI flows for app measurements, or use Playwright against the server-rendered page only when validating the public dashboard UX/security surface.
