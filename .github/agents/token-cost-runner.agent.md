...
2. Run the matching command(s) from the terminal and capture stdout:
   ...
   - Prompt cache (warm-up vs repeat, cached tokens): `mvn compile exec:java '-Dexec.mainClass=com.example.instantmodels.PromptCacheDemoApp'`
    - Deployed container app: do not POST demo API routes; the public web app intentionally exposes no `/api/*` JSON endpoints. Use the CLI flows for app measurements, or use Playwright against the server-rendered page only when validating the public dashboard UX.
