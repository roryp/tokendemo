...
2. Run the matching command(s) from the terminal and capture stdout:
   ...
   - Deployed container app: POST `"$(azd env get-value AZURE_CONTAINER_APP_URL)/api/compact-demo"` or `/api/caveman-demo` with no body — these endpoints run a fixed sample prompt and ignore any request body (the web prompt fields are read-only). For a custom prompt, use the Raw Responses API path below.
